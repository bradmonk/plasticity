function[]=postspike(varargin)
% The postspike() function takes count data of synaptic surface receptors over time.
% Using count data from these discrete timepoints, the function will inject
% a standard upstream input and compute the postmembrane potential.
%
% This function is adapted from a model provided by Eugene Izhikevich
% that reproduces spiking and bursting behavior of known types of cortical neurons. 
% The model combines the biologically plausibility of Hodgkin-Huxley-type dynamics and 
% the computational efficiency of integrate-and-fire neurons.
% http://www.izhikevich.org/publications/spikes.htm


if nargin > 0 % RUN USING FUNCTION INPUT

	[spikedata] = runspikes(varargin{1});
	...
	for sdn=1:numel(spikedata)
	set(vtrace,'xdata',spikedata{sdn}(:,1),'ydata',spikedata{sdn}(:,2));
	drawnow;
	end

	saveas(vspike,'PostSpikeOutput.png');

else % RUN AS STAND-ALONE
	
	% example data of mean surface receptor counts over discrete time-points
	varargin ={[7.2    7.4;
				13.4   16.8;
				18.6   13.4;
				20.3   11.8;
...
%--------------------------------
function[spikedata]=runspikes(varargin)
...
if exist('postspike.jpg')
    h=subplot(2,2,3);
    set(h,'Position',[0.05 0.05 0.2 0.37])
    fig1=imread('postspike.jpg');
    B=gray; mind=20;
...
%-------------------------------------------%
   ...
   xdat=tau*(L:-1:1);
   ydat=VU(1,:);
   spikedata{sn} = [xdat' ydat'];
   sn=sn+1;
end;


% functions changepars & showthreshold (for later dev)
%{
