function varargout = SAPOffRMGUI(varargin)
% SAPGUI MATLAB code for SAPGUI.fig
%      SAPGUI, by itself, creates a new SAPGUI or raises the existing
%      singleton*.
%
%      H = SAPGUI returns the handle to a new SAPGUI or the handle to
%      the existing singleton*.
%
%      SAPGUI('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SAPGUI.M with the given input arguments.
%
%      SAPGUI('Property','Value',...) creates a new SAPGUI or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before SAPGUI_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to SAPGUI_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help SAPGUI
...
handles.output = SAPOffRMGUIFUN(QRvals,params);
...
sap1 = str2double(get(handles.Bon,'String'));
sap2 = str2double(get(handles.Ron,'String'));
sap3 = str2double(get(handles.Boff,'String'));
