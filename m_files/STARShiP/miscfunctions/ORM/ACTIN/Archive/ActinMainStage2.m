%=============================================================================%
function [varargout] = ActinMainStage2(ActSteps,varargin)
...
x = Actin(:,11) * sin(TPi) * cos(PPi) + Actin(:,3);
y = Actin(:,11) * sin(TPi) * sin(PPi) + Actin(:,6);
z = Actin(:,11) * cos(TPi) + Actin(:,9);
Actin(:,4) = x;
Actin(:,7) = y;
Actin(:,10) = z;

% Actin(4,4) = 0;	Actin(5,4) = 0;
% Actin(4,7) = 0;	Actin(5,7) = 0;
% Actin(4,10) = 0;Actin(5,10) = 0;
...
%---------------------------------------------------------------%
	...
	rv=rand(6,1); % Generate a few random vaules from uniform{0:1}
		...
		if (~Xhed(aN) && ~Yhed(aN))
			...
			Actin(fNf,1) = 10;		% create branch: add 10 subunits to new branch
			Actin(fNf,11) = Actin(fNf,1) .* GaSize;
			...
			Ox = (Rmono*GaSize) * sin(Actin(aN,8)) * cos(Actin(aN,2)) + Actin(aN,3);
			Oy = (Rmono*GaSize) * sin(Actin(aN,8)) * sin(Actin(aN,2)) + Actin(aN,6);
			Oz = (Rmono*GaSize) * cos(Actin(aN,8)) + Actin(aN,9);
	...
	fDr = RotateVertex(fPr(1),fPr(2),fPr(3),fPt(1),fPt(2),fPt(3),fPo(1),fPo(2),fPo(3),...
								  fDv(1),fDv(2),fDv(3),tta);
...
% 	%=================================================%
	...
	Actin(:,4) = Actin(:,11) .* sin(Actin(:,8)) .* cos(Actin(:,2)) + Actin(:,3);
	Actin(:,7) = Actin(:,11) .* sin(Actin(:,8)) .* sin(Actin(:,2)) + Actin(:,6);
	Actin(:,10) = Actin(:,11) .* cos(Actin(:,8)) + Actin(:,9);
