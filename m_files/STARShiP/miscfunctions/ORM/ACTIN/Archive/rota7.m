function [varargout] = rota7(varargin)
...
Origin = [2; 3; 4];	% Origin of current XY
Tip = [10; 0; 10];	% Tip of current XY
Length = sqrt(sum((Tip - Origin).^2));
% Txyz1 = ARxFun(Tip, phi, tta, psy);
...
Tx = R * sin(tta) * cos(phi) + Txyz2(1);
Ty = R * sin(tta) * sin(phi) + Txyz2(2);
Tz = R * cos(tta) + Txyz2(3);
...
%				ROTATION FUNCTION
%--------------------------------------------------%
function [Rxyz] = ARxFun(xyz, phi, tta, psy)
