function [varargout] = ActinMultiplex(LBR,TIME,SIZE,DOES,REVA,AMX,MSK)
...
ArpScalar = AMX{21};
ArpDec = AMX{22};
ArpAdd = 10;		% Add X units to new branches
...
%---------------------------------------------------------------%
	...
	rv=rand(6,1); % Generate a few random vaules from uniform{0:1}
		...
		%if (ArpR*(1/(1+ exp(-linsc(Actin(aN,1)-ArpScalar))))) > rv(2)
		...
		if (~Xhed(aN) && ~Yhed(aN))
			...
			Actin(fNf,1) = ArpAdd;		% create branch: add 20 subunits to new branch
