function [] = ActinBranchMaths(varargin)
...
Txyz = (Pt - Po) ./ TL;
TL = sqrt(sum((Txyz).^2));
Ttta = acos(Txyz(3)/TL) + (pi/3);
Tphi = atan(Txyz(2)/Txyz(1)); % + (pi/6)
LTx = (TL) * sin(Ttta) * cos(Tphi);
LTy = (TL) * sin(Ttta) * sin(Tphi);
LTz = (TL) * cos(Ttta);
% LTx = (TL+1) * sin(Ttta) * cos(Tphi);
% LTy = (TL+1) * sin(Ttta) * sin(Tphi);
% LTz = (TL+1) * cos(Ttta);
Pe = [LTx;LTy;LTz]+Pt;
Pr = Pe;
...
PtPrMxX = [PMX.Dr(1,:); PMX.Pt(1,:)];
PtPrMxY = [PMX.Dr(2,:); PMX.Pt(2,:)];
PtPrMxZ = [PMX.Dr(3,:); PMX.Pt(3,:)];
