function [varargout] = SAPbrute(QRvals,params)
clc, close all
...
disp([' ' ])
disp(['The starting cluster had ' num2str(Sstart) ' particles,' ])
...
PerHourCor = OffPerStep * 3600 / 15;
Fdec = PerHourCor/Sstart;
disp([' ' ])
disp(['In 1 hour ' num2str(OffPerHour) ' saps dissociated from the cluster.' ])
disp(['...correcting for reassociation possibility (15:1 dt)...' ])
disp(['In 1 hour ' num2str(PerHourCor) ' saps exited the PSD (expected average)' ])
disp([' flourescence decrease: ' num2str(Fdec) '%' ])
disp([' flourescence remaining: ' num2str(1-Fdec) '%' ])
%-------------------------------------------%
% Noffs,Soffs,Sons
