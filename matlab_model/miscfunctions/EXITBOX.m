function [output] = EXITBOX(pbox,um)
...
%{.
if 1-exist('pbox','var')
pbox=[30 200 4 0 0.1 1000 150 70 3 2.4 0.1 3 2.4 0.1 0.2 0.2 1 1 2 1000 2 1000 8 8 1 1 1 1];
um=[3 6 .5 .5 .6 .6];
Krate=[2 1000 2 1000];
end
%}
...
GR1slotN = round(S1sum/SAPperSLOT);
GR2slotN = round(S2sum/SAPperSLOT);
GR1slotNo = GR1slotN;
GR2slotNo = GR2slotN;
...
fsizeX = round(DENXum/Scale);
fsizeY = round(DENYum/Scale);
PSD1size = round(PSD1um/Scale);
periPSD1size = round(PERI1um/Scale);
PSD2size = round(PSD2um/Scale);
periPSD2size = round(PERI2um/Scale);

[Frow Fcol row1L col1L row1F col1F PSDfield...
...
%====================%
	...
	GR2RSOT = [];
	GluR2_TdwellPSD = zeros(2,GluR2Ndots);
	G2DOSS=(G2DOSS+(randi(6)*10));
	
    %KonGR2= KonGR2o*2*rand;
    %KoffGR2= KoffGR2o*2*rand;	
%===============%
stepN = 1;
for Nt = 1:Nsteps
%===============%
    %KonGR2= KonGR2o*2*rand;
    KoffGR2= KoffGR2o+((randi(5)/10)*KoffGR2o);
	...
	TexitGR2,t,XYLBpr1,XYRTpr1,G2INPSD,G2INPSA,GR2dotsLeft,NECK);
	...
	SLOTPLOT2(GluR2xyl, Fcol, PSDLOC, PSDSZE,...
	row1F,row1L,col1F,col1L,GR2RSOT,SOTxlim,SOTylim,stepN,G2DOSS,...
	XYLBpr1,XYRTpr1,XYLBp1,XYRTp1,...
	PSD1WH,PSD2WH,PERI1WH,PERI2WH,SPYN1WH,SPYN2WH,XWIDE,YHIGH)
...
if useGluR2slots
if re == loops;GR2RSOTall(re,:) = GR2RSOT(2,:);end
GR2RSOTCell{re} = GR2RSOT(2,:);
end %if useGluR2slots
%====================%
end %for re = 1:loops
%====================%
...
for re = 1:loops
	...
	GR1RSOT = [];
	GluR1_TdwellPSD = zeros(2,GluR1Ndots);
	G1DOSS=(G1DOSS+(randi(6)*10));
	
	%KonGR1= KonGR1o*2*rand;
    %KoffGR1= KoffGR1o*2*rand;
%===============%
stepN = 1;
for Nt = 1:Nsteps
%===============%
	KoffGR1= KoffGR1o+((randi(5)/10)*KoffGR1o);
    ...
    TexitGR1,t,XYLBpr1,XYRTpr1,G1INPSD,G1INPSA,GR1dotsLeft,NECK);
	...
	SLOTPLOT2(GluR1xyl, Fcol, PSDLOC, PSDSZE,...
	row1F,row1L,col1F,col1L,GR1RSOT,SOTxlim,SOTylim,stepN,G1DOSS,...
	XYLBpr1,XYRTpr1,XYLBp1,XYRTp1,...
	PSD1WH,PSD2WH,PERI1WH,PERI2WH,SPYN1WH,SPYN2WH,XWIDE,YHIGH);


		if doLivePlot
		MAINPLOT(GluR1xyl, GluR2xyl, Fcol, PSDLOC, PSDSZE,...
...
if useGluR1slots
if re == loops;GR1RSOTall(re,:) = GR1RSOT(2,:);end
GR1RSOTCell{re} = GR1RSOT(2,:);
end %if useGluR1slots
%====================%
end %for re = 1:loops
%====================%
...
G2exit = TexitGR2all;
% Descriptive Stats
G2exitMu = mean(G2exit,1);
G2exitSD = std(G2exit,1);
G2exitSEM = G2exitSD/sqrt(size(G2exit,1));
G2exitMuSEMup = G2exitMu+G2exitSEM;
G2exitMuSEMlo = G2exitMu-G2exitSEM;

G2exitMin = min(G2exit,[],1);
G2exitMax = max(G2exit,[],1);


% ECDF Empirical Distribution and CI
[G2ECDF, G2XIT, G2CIlo, G2CIup] = ecdf(G2exitMu)
...
cdfexit = gcf;
figure(cdfexit)
subplot(5,4,[1 12]),[GluR2plot, G2Stats] = cdfplot(GluR2exT30);
%subplot(5,4,[1 12]),GluR2plot = plot(G2exitMu,1:numel(G2exitMu));
axis([xlim,ylim]);
xt = round((get(gca,'XTick'))*(t)/(60));
set(GluR2plot,'color',[1 0 1])
set(gca,'XTickLabel', sprintf('%.1f|',xt))
hold on
subplot(5,4,[1 12]),
stairs(G2XIT,G2ECDF,'MarkerFaceColor',[0.20392157137394 0.301960796117783 0.494117647409439],...
...
hold on
stairs(G2XIT,G2CIlo,'LineStyle','-.','LineWidth',1,...
	'Color',[0.39215686917305 0.474509805440903 0.635294139385223]);
stairs(G2XIT,G2CIup,'LineStyle','-.','LineWidth',1,...
	'Color',[0.39215686917305 0.474509805440903 0.635294139385223]);
hold on
...
% xlswrite('AMPARExitTime.xls',G2exit,1)
% xlswrite('AMPARExitTime.xls',G1exit,2)
...
%===========================%
    for j = 1:GR2dotsLeft  
		...
		if GluR2xyl(1,j)>=(RIT) && GluR2xyl(2,j)<=(TOP+NECK)
			GluR2xyl(1,j) = 111;
...
%===========================%
    for j = 1:GR1dotsLeft  
        ...
        if GluR1xyl(1,j)>=(RIT) && GluR1xyl(2,j)<=(TOP+NECK)
            GluR1xyl(1,j) = 111;
...
%-------------------------------------------%
function [] = SLOTPLOT2(GLURXYL, Fcol, PSDLOC, PSDSZE,...
	row1F,row1L,col1F,col1L,SOT,SOTxlim,SOTylim,stepN,DOSS,...
	XYLBpr1,XYRTpr1,XYLBp1,XYRTp1,...
	PSD1WH,PSD2WH,PERI1WH,PERI2WH,SPYN1WH,SPYN2WH,XWIDE,YHIGH)
%-------------------------------------------%
