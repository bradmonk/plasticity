function varargout = SAPMASKSGUI(varargin)
% SAPMASKSGUI MATLAB code for SAPMASKSGUI.fig
%      SAPMASKSGUI, by itself, creates a new SAPMASKSGUI or raises the existing
%      singleton*.
%
%      H = SAPMASKSGUI returns the handle to a new SAPMASKSGUI or the handle to
%      the existing singleton*.
%
%      SAPMASKSGUI('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SAPMASKSGUI.M with the given input arguments.
%
%      SAPMASKSGUI('Property','Value',...) creates a new SAPMASKSGUI or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before SAPMASKSGUI_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to SAPMASKSGUI_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help SAPMASKSGUI
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @SAPMASKSGUI_OpeningFcn, ...
                   'gui_OutputFcn',  @SAPMASKSGUI_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before SAPMASKSGUI is made visible.
function SAPMASKSGUI_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to SAPMASKSGUI (see VARARGIN)

% Choose default command line output for SAPMASKSGUI
...
% so window can get raised using SAPMASKSGUI.
if strcmp(get(hObject,'Visible'),'off')
    % plot(rand(5));
end

% UIWAIT makes SAPMASKSGUI wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = SAPMASKSGUI_OutputFcn(hObject, eventdata, handles)
...
handles.output = SAPMASKS(LBR,TIME,SIZE,MODS,DOES,REVA,GLU,GT,GTab);
% handles.output = ORMATHFUN(LBR,TIME,SIZE,MODS,DOES,REVA,GLU,GT,GTab);
