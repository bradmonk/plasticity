function [varargout] = ActinRotation(varargin)
...
for ang = 1:Fdots
	tta=ArMx(ang);
	RMX1(:,ang) = RotateVertex(x,y,z,u,v,w,tta);
end
...
for ang = 1:Fdots
	...
	RMX2(:,ang) = RotateVertex(x,y,z,a,b,c,d,e,f,u,v,w,tta);
end
...
% v0 = [x0+(x1-x0)*t0;y0+(y1-y0)*t0;z0+(z1-z0)*t0];
% v0 = [x1+(x0-x1)*t0;y1+(y0-y1)*t0;z1+(z0-z1)*t0];
% v0 = [x0+(x2-x1)*t0;y0+(y2-y1)*t0;z0+(z2-z1)*t0];
...
% Extra Plot Point (vP) and Line (vL)
vP = v0;
vL = v1;
rL = v0
...
% Pr(o) Pt(d) Dv(x)
Ph5 = scatter3(Po(1),Po(2),Po(3),'o');
set(Ph5,'SizeData',100,'LineWidth',2)
axis([-15 15 -15 15 -15 15])
hold on
Ph6 = scatter3(Pt(1),Pt(2),Pt(3),'^');
set(Ph6,'SizeData',100,'LineWidth',2)
hold on
Ph7 = scatter3(Dv(1),Dv(2),Dv(3),'p');
set(Ph7,'SizeData',100,'LineWidth',1)
hold on
Ph8 = scatter3(Pr(1),Pr(2),Pr(3),'x');
set(Ph8,'SizeData',100,'LineWidth',2)
hold on

Ph9 = scatter3(vP(1),vP(2),vP(3),'s');
set(Ph9,'SizeData',100,'LineWidth',2)
hold on
...
sFilO = cat(2,Po,sFilO);
sFilT = cat(2,vL,sFilT);

sFilO = cat(2,Pr,sFilO);
sFilT = cat(2,rL,sFilT);
...
% Plot
phAx = plot3(Xop0,Yop0,Zop0,'LineWidth',2,'MarkerSize',10);
% set(phAx,'LineStyle','-','Color',[0 0 0],'LineWidth',1);
% set(findobj('Type','line'),'Color','k')
set(phAx,{'LineWidth'},{2,2,2,1,1,1}')
hold on
%---------------------------------------------%
