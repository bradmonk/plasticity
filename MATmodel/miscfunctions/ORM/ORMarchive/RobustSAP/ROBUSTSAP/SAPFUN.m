function [varargout] = SAPFUN(QRvals,params)
clc, close all
...
% Bon = 32;		% 32 On Neighbor-Independant Rate (new growth) (lower = more on)
% Ron = 3;		% 20 On Neighbor-Dependant Rate (cluster fill-in) (higher = more on)
% 
% Loff = 1.8;	% 1.8 Off Energy (higher = more off events)
% Boff = 1.2;	% 1.2 Off Neighbor-Inependant Rate (uniform off)  (lower = more off)
...
Bon = 32;	% On Neighbor-Independant Rate (new growth) (lower = more on)
Ron = 20;	% On Neighbor-Dependant Rate (cluster fill-in) (higher = more on)

Loff = 1.8;	% Off Energy (higher = more off events)
Boff = 1.2;	% Off Neighbor-Inependant Rate (uniform off)  (lower = more off)
...
%-------------------------------------------%
OffPerHour = OffPerStep * 3600;
PerHourCor = OffPerHour / 15;
Fdec = (PerHourCor/Sstart*100) + (((Sstart - Send)/Sstart)*100);

Sdec = Sstart - Send;
PSdec = Sdec/Sstart*100;
Cdec = Sstart - PerHourCor;
PCdec = Cdec/Sstart*100;
Edec = (PCdec + Sstart)/PCdec / 10;
BradsFram = 2*(1-1/exp(Edec));

disp([' ' ])
disp(['In 1 hour ' num2str(OffPerHour) ' saps dissociated from the cluster.' ])
disp(['...correcting for reassociation possibility (15:1 dt)...' ])
disp(['In 1 hour ' num2str(PerHourCor) ' saps exited the PSD (estimated average)' ])

if (100-Fdec) > 0
disp([' flourescence decrease: ' num2str(Fdec) '%' ])
disp([' flourescence remaining: ' num2str(100-Fdec) '%' ])
else
...
% If 100 saps dis per hour, and the ending cluster size is 64
% that is a 1:1 exchange. 
