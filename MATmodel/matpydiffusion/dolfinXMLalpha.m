function [] = dolfinXMLalpha(XYZ,type,scaleval)


%% GET TRIANGULATION POINTS AND CONNECTIVITY LIST

% vrts = N x 3 dub  points
% tets = N x 4 int  connections

tets = []; vrts = [];
for nn = 1:numel(XYZ)

    alph = alphaShape(XYZ{nn}(:,1),XYZ{nn}(:,2),XYZ{nn}(:,3));
    alph.Alpha = alph.Alpha * scaleval(nn);
    [tet,vrt] = alphaTriangulation(alph);
