                  ...
                  int nrhs, const mxArray *prhs[])
{
    ...
    if( !mxIsDouble(prhs[2]) || mxIsComplex(prhs[2]) || mxGetNumberOfElements(prhs[2])!=1 ) {
        mexErrMsgIdAndTxt("advance_one_step:notScalarDouble", "k (arg 3) must be a scalar double.");
    }
	*/

	/* make sure k is double Mx1 array prhs[2] == (arg 3) */
    if( !mxIsDouble(prhs[2]) || mxGetN(prhs[2])!=1 ) {
        mexErrMsgIdAndTxt("advance_one_step:notScalarDouble", "k (arg 3) must be Mx1 double.");
    }

    /* make sure initial_point is double 1x3 array */
    if(mxGetN(prhs[3])!=3 || mxGetM(prhs[3])!=1) {
        mexErrMsgIdAndTxt("advance_one_step:notXYZVec", "initial_point (arg 4) must be 1x3.");
    }
    ...
    if(num_points!=num_k) {
        mexErrMsgIdAndTxt(
            "advance_one_step:mismatchXYZK",
            "xyz (arg 0) and k (arg 2) must have same # rows.");
    }
    ...
    plhs[0] = mxDuplicateArray(prhs[0]); // xyz
    plhs[1] = mxDuplicateArray(prhs[1]); // face
    plhs[2] = mxDuplicateArray(prhs[2]); // k

    xyz_loc = mxGetPr(plhs[0]);
    face_indices = (long *)mxGetData(plhs[1]);
    k = mxGetData(plhs[2]);

    advance_one_step_c((mwSize) num_points, (mwSize) num_vertices, (mwSize) num_triangles,
                       xyz_loc, face_indices, k, initial_point, initial_face_index, all_vertices,
                       triangles, face_local_bases, neighbor_faces);
